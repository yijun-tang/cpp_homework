...
void Warehouse<T>::addProduct(std::shared_ptr<Product<T>> product) {
    ...
    if (products.find(id) != products.end()) {
        throw WarehouseException("Product with ID " + std::to_string(id) +
                                 " already exists.");
    }
...
template <typename T>
void Warehouse<T>::updateProduct(size_t id, const std::string& name, T price,
                                 int quantity) {
    if (quantity < 0) {
...
void supplierUpdate(Warehouse<T>& warehouse, size_t productID, int minQuantity,
    ...
    try {
        ...
        if (product) {
            ...
            std::cout << "Supplier updated Product ID " << productID << " by "
                      << quantityChange
...
void inventoryCheck(Warehouse<T>& warehouse, int checkInterval,
    ...
    while (true) {
        ...
        try {
            std::cout << "Checking inventory for low stock items..."
                      << std::endl;
            ...
            for (auto it = warehouse.cbegin(); it != warehouse.cend(); ++it) {
                ...
                if (product->getQuantity() < lowStockThreshold) {
                    std::cout
                        << "Low stock alert! Product ID: " << product->getID()
...
template void Warehouse<int>::updateProduct(size_t, const std::string&, int,
                                            int);
